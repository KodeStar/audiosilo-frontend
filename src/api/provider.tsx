...
// on the query cache rather than each call site. The connection id is resolved from the
// key by membership against the live connection ids (keys are connection-scoped but not
// at a fixed index).
...
function noteAuthError(error: unknown, key: readonly unknown[]) {
  ...
  const { connections, markNeedsReconnect } = useSession.getState();
  const cid = connectionIdFromKey(
    key,
    connections.map((c) => c.id),
  );
  if (cid) markNeedsReconnect(cid, 'auth');
}
