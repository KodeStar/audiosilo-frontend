 ...
 * account has neither a password nor a recovery code; an unknown flag is treated as
 * "don't warn" rather than a guess that would false-positive. The caller
 * (`useSignOut`) refreshes the flags from /me before deciding when they're unknown,
 * so this only stays unknown when the server is unreachable — and offline a warning
 * would be a dead end anyway (a recovery code can't be minted without the server).
 */
