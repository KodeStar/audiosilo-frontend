...
2. Add or edit a line in [`manifest.mjs`](manifest.mjs), e.g.
   ...
   automatically.
3. Regenerate (needs your FontAwesome Pro token):

   ```sh
   cd scripts/glyphs
   set -a; . ../../.env; set +a   # loads FONTAWESOME_NPM_AUTH_TOKEN from the repo .env
...
4. Commit the updated `icon-data.ts`. **Do not** commit this folder's
   `node_modules` or `package-lock.json` (both gitignored) — they would re-pin the
   private registry for everyone.
